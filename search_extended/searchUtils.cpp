...
void Utils::addToAnswer(ScMemoryContext &ctx, const ScAddr &answer, const ScAddr &elem, sc_bool sys_off = SC_TRUE)
{
    if (SC_TRUE == sys_off)
    {
    ...
    if (!ctx.HelperCheckArc(answer, elem, ScType::EdgeAccessConstPosPerm))
        ctx.CreateArc(ScType::EdgeAccessConstPosPerm, answer, elem);
}

sc_bool isSubset(ScMemoryContext & ctx, ScAddr const &parent_set, ScAddr const &child_set)
{
